...
def load_risks(path: Path) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(
            f"{path} nicht gefunden. "
            "Lauf zuerst preprocessing/risks/parquet-transformer.py (app/pipelines/nli/preprocessing)."
        )
    df = pd.read_parquet(path)
    if TEXT_COL not in df.columns:
        raise ValueError(
            f"Spalte '{TEXT_COL}' nicht im Risk-Parquet gefunden. "
            f"Spalten: {list(df.columns)}"
        )
    if ID_COL not in df.columns:
        raise ValueError(
            f"Spalte '{ID_COL}' nicht im Risk-Parquet gefunden. "
            f"Spalten: {list(df.columns)}"
        )
    return df
...
def main() -> None:
    print(f"➡️ Lade Risks aus {RISKS_FILE}")
    df = load_risks(RISKS_FILE)
    print(f"✅ {len(df)} Risks geladen.")
    ...
    meta = df[[ID_COL, TEXT_COL, "risk_name", "risk_type", "segment", "region"]].copy()
    meta.to_parquet(META_FILE, index=False)
    print(f"✅ Fertig: {len(df)} Risks im Index.")
